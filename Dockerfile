...
RUN apt-get update && \
    apt-get install -y --no-install-suggests --no-install-recommends ca-certificates git python3 python3-dev python3-distutils python3-numpy libxml2 libxml2-dev libsnappy1v5 libsnappy-dev make gcc g++ curl && \
    apt-get clean && \
